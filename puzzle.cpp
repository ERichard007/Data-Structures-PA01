...
and set members rows/cols accordingly Tested by: unit_tests/unit_test_0
Post-conditions:
...
void Puzzle::shift_row(int row, bool reverse) {
  if (reverse == false) {
    char lastChar = symbols[row][cols - 1]; // i = 2, 1 2 3 --- 1 2 2 --- i = 1,
                                            // 1 1 2 --- i = 0 ...
    for (int i = cols - 1; i > 0; i--) {
      symbols[row][i] = symbols[row][i - 1];
    }
    symbols[row][0] = lastChar; // 3 1 2
  } else {
    char firstChar =
        symbols[row][0]; // i=0, 1 2 3 --- 2 2 3 --- i=1, 2 3 3 --- i = 2...
    for (int i = 0; i < cols - 1; i++) {
      symbols[row][i] = symbols[row][i + 1];
    }
    symbols[row][cols - 1] = firstChar;
  }
}
...
void Puzzle::shift_col(int col, bool reverse) {
  if (reverse == false) {
    char lastChar = symbols[rows - 1][col];
    for (int i = rows - 1; i > 0; i--) {
      symbols[i][col] = symbols[i - 1][col];
    }
    symbols[0][col] = lastChar;
  } else {
    char firstChar = symbols[0][col]; // 1 2 3 --- 2 2 3 --- 2 3 3 ---
    for (int i = 0; i < rows - 1; i++) {
      symbols[i][col] = symbols[i + 1][col];
    }
    symbols[rows - 1][col] = firstChar;
  }
}
